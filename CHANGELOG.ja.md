...
## 0.1.4
- サウンドを追加：Pyxel の 4 チャンネルで MML を鳴らす `PLAY`
- チャンネルが再生中かを返す `PLAY(ch)` 関数を追加
- スプライトを追加：Graphic面 と Text面 の間に Sprite面 を追加
- `&H` 接頭辞による16進整数リテラルを追加（例: `&HFF`）
- `HEX$(n)` 関数を追加
- 関数を引数の個数が合わない状態で呼んだときにクラッシュするのを修正
- `CLS` のマスクが 1-3 の範囲外のとき、エラーになるように修正
- サンプル `play.bas` を追加
- サンプル `sprite.bas` を追加
- サンプル `jumpman.bas` を追加

## 0.1.3
- ブロック崩しサンプル `brickbreaker.bas` を追加
- thread モードで `POINT` がクラッシュするバグを修正
